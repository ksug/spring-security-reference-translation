...
[[what-is-acegi-security]]
=== acegi 시큐리티란 무엇인가
여러분이 아마 아시다시피, 어플리케이션보안의  두 주요 부분은 인증(authentication)과 인가(authorization 혹은 접근제어access-control) 입니다. 인증과 인가는 스프링 시큐리티가 목표로하는 두 주요 부분입니다. 인증(Authentication) 은 접근주체(principal)을 만드는 과정입니다. 여기서 접근주체란 일반적으로 당신의 어플리케이션에서 어떠한 행동을 하려하는 사용자나 디바이스 혹은 어떤 다른 시스템을 말합니다. 

인가(Authroization)은 어떤 접근주체(Principal)이 당신의 어플리케이션에서 어떠한 행동을 취할 수 있는 것이 허용되는지 아닌지 에 대한 것을 결정하는 과정을 나타냅니다. 인가결정이 필요한 지점에 이르기 위해 접근주체의 식별자는 인증 과정에서 이미 설정과되어있어야 합니다. 이러한 과정은 스프링 시큐리티만의 특별한 과정이 아닙니다.

인가를 하기 위해서, 스프링 시큐리티는 다양한 인증모델을 지원하는데, 대부분의 이러한 인증 모델은 서드파티에 의해 제공되거나 인터넷 엔지니어링 전담반같은 관련된 표준부처에 의해 개발됩니다. 추가적으로 스프링 시큐리티는 스프링 시큐리티 고유의 인증 특징들을 제공합니다. 특별히 스프링 시큐리티는 이러한 모든 기술들과의 통합을 지원합니다.
////
...
많은 독립적인 소프트웨어 공급자들이 스프링 시큐리티를 사용한 이유는 이 유연한 인증모델때문입니다. 그렇게 함으로써, 그들의 end client 가 필요로 하는 것이 무엇이건간에 많은 엔지니어링이나 클라이언트의 큰 환경 변화없이 그들은 빠르게 그들의 솔루션을 통합할 수가 있었습니다.  만약 당신의 필요에 위의 인증 메커니즘이 해당하지 않는다면, 스프링 시큐리티는 오픈플랫폼이므로, 당신의 인증 메커니즘을 적는 것은 무척 간편합니다. 스프링 시큐리티의 많은 기업사용자들은, 어떤 특정 보안표준을 따르지 않는 레거시 시스템과 통합을 원하고 있고 스프링 시큐리티는 꽤 이러한 시스템과 잘 맞아떨어집니다(play nicely)
...
인증 모델과는 상관없이, 스프링 시큐리티는 인가에 관한 탁월한 기능들을 제공하는데 , 웹요청을 인가하고, 어떤 메소드가 실행가능한지 인가하고, 독립적인 도메인 오브젝트 인스턴스에 접근하는 것에 대해 인가합니다. 이러한 것들의 차이를 이해하기 위해 각각의 파일 시스템 보안이나 EJB 컨테이너관리보안이나 서블릿 규격 웹패턴 보안에 나타나는 기능들을 살펴보시면 스프링 시큐리티가 이러한 모든 영역에서 탁월한 기능들을 제공하는 것을 아실 수 있을 것입니다 . 우리는 이 레퍼런스 가이드 문서에서 더 자세히 살펴볼 것입니다 .
////
...
=== 역사
스프링 시큐리티는 2003년 말에 "The Acegi Security System for Spring" 로써 시작되었습니다. 스프링개발자들의 메일리스트에서 스프링 기반의 시큐리티 구현체를 고려중이라는 질문이 나타났습니다. 스프링 커뮤니티에서는 당시 상대적으로 작았습니다. (오늘에 비교해서 말입니다) . 그리고 실제로 스프링 자신은 2003년초에 소스포지 프로젝트로써 존재할 뿐이었습니다. 이 질문에 대한 응답은 비록 시간이 좀 부족해도, 해볼만한 영역이라는 응답이었습니다. 

그러한 점을 염두에 두고, 간소한 시큐리티 구현체가 만들어졌지만 릴리즈되지는 않았습니다. 스프링 커뮤니티의 다른 멤버가 몇 주후에 시큐리티에 관해 문의를 하였고, 당시 코드가 그들에게 제공되었습니다. 몇몇 다른 요청들이 뒤따라왔고, 2004년 1월 거의 20명의 사람들이 코드를 사용하고 있었습니다. 소스포즈 프로젝트에 제안한 개척자들이 순서대로 합류하였으며, 예상대로 2004년 3월 자리를 잡기 시작합니다(established) 
...
초기에는 프로젝트는 자체적인 인증 모듈이 없었습니다. 시큐리티에 관리되는 콘테이너는 인증에 집중하기보다는 Acegi 시큐리티한 인증 처리 프로세스에 의존하였습니다. 이것은 처음에 적합했지만 점점 더 많은 사용자들의 추가적인 콘테이너 지원을 요청하였습니다. 콘테이너특징적인 인증 인터페이스 영역의 근본적인 한계는 뚜렷했습니다. 또, 콘테이너의 클래스패스에 새로운 JAR를 추가해야되는 것과 관련된 이슈또한 있었고, 이는 최종 사용자의 혼란과 잘못된 설정의 주된 요인이었습니다. 
